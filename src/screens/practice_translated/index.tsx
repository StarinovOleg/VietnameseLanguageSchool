...
import { translated, trans_1, trans_2 } from "../../dictionary/unit-1/translated";
...
function PracticeTranslatedIndex() {
    ...
    const submit=(e:any)=>{
        ...
        if(trans_1[1].vietnamese===message){
            console.log('good2')
            setMessage('')
        }else if(trans_1[1].vietnamese!=message || message.length<=0){  
            console.log('wrong2')
            setMessage('')
        };
    }
    const handleChange = (event: { target: { value: React.SetStateAction<string>; }; }) => {
      ...
      const display_vietnamese_word=()=>trans_1.slice(step - 1, step).map((step, index) => {
        return(
        <>
        <H1
          children={step.english}
          key={index}
        />
        <input onChange={handleChange} value={message} type="text" id="first_name" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Here your answer" required/>
        </>
        )
      })
    const displayWords = () => {
    ...
    const handleClick = (event:any) => {
      ...
      setLoading(true);
       ...
       if(trans_1[1].vietnamese===inputMessage1){
        ...
        }else if(trans_1[1].vietnamese!=inputMessage1 || inputMessage1.length<=0){  
            console.log('wrong1')
            setActiveNotCorrect1(!active_notcorrect1)
        };
        //block-3
       if(trans_1[2].vietnamese===inputMessage2){
        ...
        }else if(trans_1[2].vietnamese!=inputMessage2 || inputMessage2.length<=0){  
            console.log('wrong1')
        ...
        if(trans_1[3].vietnamese===inputMessage3){
            ...
            }else if(trans_1[3].vietnamese!=inputMessage3 || inputMessage3.length<=0){  
                console.log('wrong1')
        ...
        if(trans_1[4].vietnamese===inputMessage4){
            ...
            }else if(trans_1[4].vietnamese!=inputMessage4 || inputMessage4.length<=0){  
                console.log('wrong1')
        ...
        if(trans_1[5].vietnamese===inputMessage5){
            ...
            }else if(trans_1[5].vietnamese!=inputMessage5 || inputMessage5.length<=0){  
                console.log('wrong1')
        ...
        if(trans_2[0].english===inputMessage6){
          ...
          }else if(trans_2[0].english!=inputMessage6 || inputMessage6.length<=0){  
        ...
        if(trans_2[1].english===inputMessage7){
          ...
          }else if(trans_2[1].english!=inputMessage7 || inputMessage7.length<=0){  
              console.log('wrong1')
        ...
        if(trans_2[2].english===inputMessage8){
          ...
          }else if(trans_2[2].english!=inputMessage8 || inputMessage8.length<=0){  
              console.log('wrong1')
        ...
        if(trans_2[3].english===inputMessage9){
          ...
          }else if(trans_2[3].english!=inputMessage9 || inputMessage9.length<=0){  
              ...
              return (
                  <>
                    ...
                    <Button
                        ...
                        btnprimary={`${loading ? 'cursor-no-drop border-stone-500 text-stone-500' : 'cursor-pointer md:hover:border-sky-800 lg:hover:border-sky-800 lg:hover:text-sky-800 md:hover:text-sky-800'} border-2 border-sky-500  rounded-lg  lg:text-5xl md:text-5xl text-xl text-sky-500 `}
                        disabled={loading}
                    />
                  <div className="text-left  " >    
                      <H2 children={translated.translated_1_1.title} fontsizeprimary="lg:text-5xl md:5xl text-xl mt-10 mb-10"/>
                      <div className="mt-4">
                      <Input onChange={handleChange}  value={message} sentence={trans_1[0].english} active_correct={active_correct} active_notcorrect={active_notcorrect}/>
                      <Input onChange={handleChange1} value={inputMessage1} sentence={trans_1[1].english} active_correct={active_correct1} active_notcorrect={active_notcorrect1}/>
                      <Input onChange={handleChange2} value={inputMessage2} sentence={trans_1[2].english} active_correct={active_correct2} active_notcorrect={active_notcorrect2}/>
                      <Input onChange={handleChange3} value={inputMessage3} sentence={trans_1[3].english} active_correct={active_correct3} active_notcorrect={active_notcorrect3}/>
                      <Input onChange={handleChange4} value={inputMessage4} sentence={trans_1[4].english} active_correct={active_correct4} active_notcorrect={active_notcorrect4}/>
                      <Input onChange={handleChange5} value={inputMessage5} sentence={trans_1[5].english} active_correct={active_correct5} active_notcorrect={active_notcorrect5}/>

                      </div>
                  </div>
                  <div className="text-left">    
                      <H2 children={translated.translated_1_2.title} fontsizeprimary="lg:text-5xl md:5xl text-xl mt-10 mb-10"/>
                      <div className="mt-4">
                      <Input onChange={handleChange6} value={inputMessage6} sentence={trans_2[0].vietnamese} active_correct={active_correct6} active_notcorrect={active_notcorrect6}/>
                      <Input onChange={handleChange7} value={inputMessage7} sentence={trans_2[1].vietnamese} active_correct={active_correct7} active_notcorrect={active_notcorrect7}/>
                      <Input onChange={handleChange8} value={inputMessage8} sentence={trans_2[2].vietnamese} active_correct={active_correct8} active_notcorrect={active_notcorrect8}/>
                      <Input onChange={handleChange9} value={inputMessage9} sentence={trans_2[3].vietnamese} active_correct={active_correct9} active_notcorrect={active_notcorrect9}/>
                      </div>
                  </div>
                  </>
              );
